...
class TcgaData(data.Dataset):
    def __init__(self, dataset_cfg=None,state=None):
        ...
        # ---->split dataset
        if state == 'train':
            self.data = self.slide_data.loc[:, 'train_slide_id'].dropna()
            self.case_id = self.slide_data.loc[:, 'train_case_id'].dropna()
            self.label = self.slide_data.loc[:, 'train_label'].dropna()
        if state == 'val':
            self.data = self.slide_data.loc[:, 'val_slide_id'].dropna()
            self.case_id = self.slide_data.loc[:, 'val_case_id'].dropna()
            self.label = self.slide_data.loc[:, 'val_label'].dropna()
        if state == 'test':
            self.data = self.slide_data.loc[:, 'test_slide_id'].dropna()
            self.case_id = self.slide_data.loc[:, 'test_case_id'].dropna()
